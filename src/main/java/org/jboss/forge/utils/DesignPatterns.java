...
public class DesignPatterns {
    ...
    public static JavaClassSource createBuilder(JavaClassSource javaClass) {
        ...
        String objName = javaClass.getName().substring(0,1).toLowerCase()+javaClass.getName().substring(1);
        builderClass.addField().setPrivate().setFinal(true).setType(javaClass).setName(objName)
        ...
        builderClass.addMethod()
                .setPublic().setReturnType(javaClass).setName("build").setBody("return " +
                "" +objName +
                ";");
        for (PropertySource<JavaClassSource> property : javaClass.getProperties()) {
            ...
            method.setBody("" +
                    "" + objName +
                    ".set" + upperCasedName + "(param);return this;");
        }
        return builderClass;
    }
